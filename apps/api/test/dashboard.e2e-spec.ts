...
describe('Intel /dashboard', () => {
  ...
  beforeAll(async () => {
    ...
    const c = await request(http).post('/api/v1/intel/competitors').send({ name: 'A', groupId: g.body.id });
    await request(http)
  ...
  it('KPI: 3 fiyatlı ürün, 1 zararına', async () => {
    const res = await request(http).get(`/api/v1/intel/dashboard?date=${DATE}`).expect(200);
