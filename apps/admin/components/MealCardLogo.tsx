 ...
 * inline SVG wordmark'lar (harici görsel/CDN yok, CSP güvenli, keskin/vektörel).
 * Nakit/kart için basit çizgi ikon. `h` yükseklik (px); genişlik orantılı.
 */
export type PayId = 'CASH' | 'CARD' | 'SETCARD' | 'MULTINET' | 'TOKENFLEX' | 'EDENRED' | 'METROPOL';
...
/** Yemek kartı markaları — logolarındaki baskın renk/biçim. */
export function MealCardLogo({ id, h = 18 }: { id: PayId; h?: number }) {
  switch (id) {
    case 'MULTINET':
      // Multinet Up — turuncu-kırmızı geçiş, koyu lacivert metin.
      return <Wordmark text="multinet" bg="#f47216" fg="#0b1f3a" h={h} />;
    case 'SETCARD':
      // SetCard — mavi zemin, beyaz metin.
      return <Wordmark text="setcard" bg="#0a5bb5" h={h} />;
    case 'EDENRED':
      // Edenred — lacivert marka rengi.
      return <Wordmark text="edenred" bg="#12285a" h={h} />;
    case 'METROPOL':
      // Metropol Card — kırmızı.
      return <Wordmark text="metropol" bg="#e0121b" h={h} />;
    case 'TOKENFLEX':
      // Token Flex (Edenred) — mor tonları.
      return <Wordmark text="token flex" bg="#5b2a86" h={h} />;
    case 'CARD':
      return (
